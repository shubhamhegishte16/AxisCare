...
const Medicines = () => {
  ...
  const handleAdd = (e) => {
    ...
    const newMed = {
      ...
      name: form.name || 'Untitled Medicine',
      ...
      status: Number(form.quantity) > 0 ? 'In Stock' : 'Out of Stock',
    };
  ...
  const handleDelete = (id) => setMedicines(medicines.filter((m) => m.id !== id));
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans">
      ...
      <main className="flex-1 p-6 lg:p-8 max-w-7xl mx-auto w-full">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-5 mb-6">
          <StatCard title="TOTAL MEDICINES" value={stats.total} icon={Package} iconColor="text-blue-600" bgColor="bg-blue-50" />
          ...
          <StatCard title="EXPIRING SOON" value={stats.expiringSoon} icon={CalendarClock} iconColor="text-orange-500" bgColor="bg-orange-50" />
        </div>
        ...
        <Card>
          ...
          <div className="overflow-x-auto">
              ...
              <tbody>
                {filtered.map((m) => (
                  <tr key={m.id} className="border-b border-gray-50 hover:bg-gray-50/60">
                    ...
                    <td className="py-3 text-gray-500">{m.supplier}</td>
                    ...
                    <td className="py-3">
                      <div className="flex items-center justify-end gap-3">
                        ...
                        <button onClick={() => handleDelete(m.id)} className="text-gray-400 hover:text-red-600"><Trash2 className="w-4 h-4" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
            ...
            {filtered.length === 0 && <EmptyState text="No medicines match your search." />}
          </div>
        </Card>
      </main>
      ...
      <Modal open={showAdd} onClose={() => setShowAdd(false)} title="Add Medicine" wide>
        <form onSubmit={handleAdd} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="sm:col-span-2 flex justify-end gap-3 pt-2">
            ...
            <button type="submit" className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold px-5 py-2.5 rounded-lg shadow-sm">
              <PackagePlus className="w-4 h-4" /> Save Medicine
            </button>
          </div>
        </form>
      </Modal>
      ...
      <Modal open={!!viewItem} onClose={() => setViewItem(null)} title={viewItem?.name}>
        {viewItem && (
          <div className="space-y-3 text-sm">
            ...
            <Row label="Expiry Date" value={viewItem.expiry} />
